...
export default function () {
  ...
  const isSelectAll = useMemo(() => localSelectKeys.length === Object.keys(localData).length, [localSelectKeys, localData])
  ...
  return (
    <div style={{width: 450, padding: '12px 16px'}}>
      <Space size={16} style={{marginBottom: 16}}>
        <Button type={'primary'} onClick={switchCache}>切换</Button>
        <Button type={'primary'} onClick={saveCache}>保存</Button>
      </Space>
      <Space style={{width: '100%'}} direction={'vertical'}>
        <div style={{display: 'flex', justifyContent: 'space-between'}}>
          ...
          <Space>
            {isCache && <a
              onClick={() => setLocalSelectKeys(isSelectAll ? [] : Object.keys(localData))}>{isSelectAll ? '反选' : '全选'}</a>}
